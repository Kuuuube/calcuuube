...
pub fn test_calculation_outputs() {
    let mut parser_context = fend_core::Context::new();
    ...
    assert!(calculate_string_to_string("1/3", &mut parser_context).unwrap() == "0.3333333333".to_owned());
    ...
    assert!(calculate_string_to_string("√", &mut parser_context) == None);
    assert!(calculate_string_to_string("10*1000000000000000", &mut parser_context).unwrap() == "10000000000000000".to_owned());
    assert!(calculate_string_to_string("10*1000000000000000000000000000000000000000000000000000", &mut parser_context).unwrap() == "10000000000000000000000000000000000000000000000000000".to_owned());
    ...
    assert!(calculate_string_to_string("log₁₀(9)", &mut parser_context).unwrap() == "0.9542425094".to_owned());
    ...
    assert!(calculate_string_to_string("log10(10)", &mut parser_context).unwrap() == "1".to_owned());
    assert!(calculate_string_to_string("log3(3)", &mut parser_context).unwrap() == "1".to_owned());
}
